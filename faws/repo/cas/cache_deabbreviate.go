...
// abbreviation must be valid hexadecimal and lowercase
func (cache *cache) Deabbreviate(abbreviation string) (hash ContentID, err error) {
	if len(abbreviation) == 1 {
		var candidate string
		var dir_entries []os.DirEntry
		dir_entries, err = os.ReadDir(cache.directory)
		if err != nil {
			return
		}
	...
	if len(abbreviation) == 2 {
		bucket1_path := filepath.Join(cache.directory, abbreviation)
		var dir_entries []os.DirEntry
	...
	if len(abbreviation) == 3 {
		bucket1_path := filepath.Join(cache.directory, abbreviation[0:2])
		var dir_entries []os.DirEntry
		dir_entries, err = os.ReadDir(bucket1_path)
		if err != nil {
			err = ErrObjectNotFound
	...
	if len(abbreviation) == 4 {
		bucket2_path := filepath.Join(cache.directory, abbreviation[0:2], abbreviation[2:4])
		var dir_entries []os.DirEntry
	...
	if len(abbreviation) < 4 {
		err = fmt.Errorf("%w: %s", ErrObjectNotFound, abbreviation)
		return
	}
	...
	bucket := filepath.Join(cache.directory, abbreviation[0:2], abbreviation[2:4])
