...
func (cache *cache) List(fn ListFunc) (err error) {
	var id ContentID
	var buckets []os.DirEntry
	var bucket2s []os.DirEntry
	var objects []os.DirEntry
	var name_err error
	buckets, err = os.ReadDir(cache.directory)
	if err != nil {
		console.Println(cache.directory)
		return
	}
	...
	for _, bucket := range buckets {
		if bucket.IsDir() && len(bucket.Name()) == 2 {
			...
			if name_err == nil {
				bucket1path := filepath.Join(cache.directory, bucket.Name())
